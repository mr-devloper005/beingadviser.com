...
import { ArrowRight, Building2, Compass, LayoutGrid, ShieldCheck } from 'lucide-react'
...
import { getListingTaskOrNull } from '@/components/shared/ui-listing-only-nav'
...
const taskIcons: Partial<Record<TaskKey, typeof Building2>> = {
  listing: Building2,
}
...
function DirectoryHome({ primaryTask, enabledTasks, listingPosts, classifiedPosts, profilePosts, brandPack }: {
  ...
  const secondaryPosts = profilePosts.length
    ? profilePosts
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-14 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
          <div className={`rounded-[2rem] p-7 ${tone.panel}`}>
            ...
            <ul className={`mt-6 space-y-3 text-sm leading-7 ${tone.muted}`}>
              ...
              <li>More listing surfaces when you want to explore beyond the featured row.</li>
            </ul>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {secondaryPosts.slice(0, 4).map((post) => {
              const meta = getPostMeta(post)
              const taskKey: TaskKey = 'listing'
              return (
                <Link key={post.id} href={getTaskHref(taskKey, post.slug)} className={`overflow-hidden rounded-[1.8rem] ${tone.panel}`}>
                  ...
                  <div className="p-5">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.24em] opacity-70">{meta.category || 'Listing'}</p>
...
export default async function HomePage() {
  ...
  const listingTask = getListingTaskOrNull()
  const enabledTasksForUi = listingTask ? [listingTask] : []
  const listingPosts = listingTask
    ? await fetchTaskPosts('listing', 8, { allowMockFallback: false, fresh: true })
    : []
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <DirectoryHome
        primaryTask={listingTask ?? undefined}
        enabledTasks={enabledTasksForUi}
